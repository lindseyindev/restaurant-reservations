...
function Dashboard({ date }) {
  ...
  function statusChanger(reservation_id, status) {
    const abortController = new AbortController();
    updateStatus(reservation_id, status, abortController.signal).catch(setError)
    return () => abortController.abort()
  }
  ...
  const displayReservations = reservations.map((reservation) => {
    ...
    return (
      <div className="p-2 m-4 hover:bg-yellow-200">
        ...
        ><button onClick={(e) => statusChanger(reservation.reservation_id, "seated")} className="mt-2 bg-white hover:bg-gray-100 text-gray-800 font-semibold py-2 px-4 border border-gray-400 rounded shadow">
          Seat</button>
        </a> : null}
      </div>
    );
  ...
  return (
    <main className="min-h-screen m-12">
      ...
      <div className="flex flex-row">
        ...
        <div className="p-4 m-5 bg-indigo-200"> Tables
